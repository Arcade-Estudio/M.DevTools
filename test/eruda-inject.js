// ============================================================
// ErudaArcade Plugin Injector (standalone, no SW required)
// ============================================================
// AppTab unificado con sub-tabs tipo navegador:
//  App  |  NetWat  |  Elements  |  Inspect
// Scroll interno en cada panel
// ============================================================
...
(function(){
  ...
  // Patch global de addEventListener para Elements+
  ...
  function createAppTab(eruda) {
    ...
    function resolveCode(fn) {
      return fn.toString().replace(/^function\s*\([^)]*\)\s*{/, '').replace(/}\s*$/, '')
        .replace(/this\._render/g, 'var rd=eruda.get(\'AppTab\');rd._render')
        .replace(/this\._refresh/g, 'var rf=eruda.get(\'AppTab\');rf._refresh')
    }
...
')
      ...
      render: function() {
        ...
        // --- App panel ---
        s += '<div class="eruda-apptab-panel' + (activeTab === 'App' ? ' active' : '') + '" data-panel="App">'
        s += '<div class="eruda-apptab-inner">' + this._renderAppSections() + '</div></div>'

        // --- NetWat panel ---
        s += '<div class="eruda-apptab-panel' + (activeTab === 'NetWat' ? ' active' : '') + '" data-panel="NetWat">'
        s += '<div class="eruda-netwat-list">' + this._renderNetwatRows() + '</div></div>'

        // --- Elements panel ---
        s += '<div class="eruda-apptab-panel' + (activeTab === 'Elements' ? ' active' : '') + '" data-panel="Elements">'
        s += '<div class="eruda-eplus-list">' + this._renderElementsContent() + '</div></div>'

        // --- Inspect panel ---
        s += '<div class="eruda-apptab-panel' + (activeTab === 'Inspect' ? ' active' : '') + '" data-panel="Inspect">'
        s += this._renderInspectHeader()
        ...
        // Start sensors if App tab active
      ...
      _renderNetwatRows: function() {
        ...
        var s = '<div class="eruda-netwat-header">'
          ...
          + '<tr><th>URL</th><th>Tipo</th><th>Tama\u00f1o</th><th style="width:180px">Waterfall</th><th>Total</th></tr>'
      ...
      // OBJINSPECT TAB
      // ========================
      _renderInspectHeader: function() {
        return '<div style="padding:6px;display:flex;gap:4px">'
          + '<input id="eruda-obj-input" class="eruda-objtree-input" placeholder="Ej: window, document, location, navigator..." value="window">'
          + '<button class="eruda-objtree-btn" onclick="var at=eruda.get(\'AppTab\');var v=document.getElementById(\'eruda-obj-input\').value;try{at._doInspect(eval(\'(\'+v+\')\')||eval(v),v)}catch(e){at._showInspectError(e.message)}">Inspect</button>'
          + '</div>'
      },
  ...
  waitForEruda(function(eruda) {
    // Solo registrar AppTab (unificado)
    if (!eruda.get('AppTab')) {
